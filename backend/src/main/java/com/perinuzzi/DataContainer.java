...
public class DataContainer implements Serializable{
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    public DataContainer(String testFilePath, boolean test) {
        if (test) {
            try {
                this.data = loadDataset(testFilePath, 200);
            } catch (IOException e) {
                e.printStackTrace();
            }
            this.rows = 200; 
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    private static String[][] loadDataset(String source, int rows) throws IOException {
        try (BufferedReader br = new BufferedReader(new FileReader(source))) {
            br.readLine(); // Consume header to skip so we don't load it into our data
            String line;
            int row = 0;
            String dataset[][] = new String[rows][13];   // Add an extra column (13) here for label placement
